<?php
	...
	if(isset($postdata))
	{
	 ...
	 //GET ONLINE DOCTORS AND SPECIALITY WISE DETIAILS FROM SPECIAL,DOCTORDETAILS AND DOCTOR ONOFF TABLE
	  $sql="select count(donoff.onoff) as noofonlinedoctors,spec.id,spec.special,spec.content,spec.description from doctorDetails as dd LEFT JOIN speciality as spec ON dd.doctorSpecialityId=spec.special LEFT JOIN doctor_onoff as donoff ON donoff.doctor_phno=dd.doctorPhone  where spec.special like '%$sp%'";
