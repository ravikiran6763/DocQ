...
// echo "select docImage2,doctorFname,doctorMname,doctorLname,doctorDetails.doctorPhone,consultationId,consultDate,message,date(dateAndTime) as date,unreadCount from myConsultations,doctorDetails,chatHistory,doctorImages where patientPhone='$loginphno' and doctorImages.docPhone=doctorDetails.doctorPhone and myConsultations.doctorPhone=doctorDetails.doctorPhone and chatHistory.chatFrom=myConsultations.doctorPhone  and dateAndTime=(select max(dateAndTime) from chatHistory where  chatFrom=myConsultations.doctorPhone) group by myConsultations.doctorPhone";
